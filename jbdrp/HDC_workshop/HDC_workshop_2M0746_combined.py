...
if __name__ == "__main__":
    ...
    # rv_list = np.arange(-18,60,1)[:,None]
    ...
    if 1:
        ...
        out = os.path.join(data_dir,"out", "2M0746"+BD+"_combined_rvs.fits")
        try:
        ...
        out = os.path.join(data_dir,"out", "2M0746"+BD+"_combined_dAIC.fits")
        try:
            hdulist.writeto(out, overwrite=True)
        except TypeError:
            hdulist.writeto(out, clobber=True)
        ...
        out = os.path.join(data_dir,"out", "2M0746"+BD+"_combined_ampl.fits")
        try:
            hdulist.writeto(out, overwrite=True)
        except TypeError:
            hdulist.writeto(out, clobber=True)
        ...
        out = os.path.join(data_dir,"out", "2M0746"+BD+"_combined_logpost.fits")
        try:
            hdulist.writeto(out, overwrite=True)
        except TypeError:
            hdulist.writeto(out, clobber=True)
    ...
    plt.xlabel("RV (km/s)")
    plt.ylabel("log(L1/L0)")
    plt.subplot(1,3,2)
    plt.plot(rv_list,out_ampl)
    plt.xlabel("RV (km/s)")
    plt.ylabel("planet amplitude (~CCF)")
    plt.subplot(1,3,3)
    plt.plot(rv_list,np.exp(out_logpost-np.max(out_logpost)))
    plt.xlabel("RV (km/s)")
    plt.ylabel("RV posterior")
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, "out", "2M0746"+BD+"_combined_plot.png"), bbox_inches='tight')
    plt.show()
