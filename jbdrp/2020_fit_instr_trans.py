...
def instr_trans_model(paras, x,wvs0,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_model,rv):
    ...
    tmp /= np.nanmax(tmp)
    # print(tmp[1000],instr_trans[1000],phoenix_HR8799_func(wvs0*(1-rv/c_kms))[1000],atran_2d_func(wvs0, water)[1000],water)
...
def _fit_instrument_transmission(paras):
    ...
    paras0 = np.array((specmax*np.ones(deg_model+1)).tolist() + [2000])
    ...
    # print(instr_trans_model(out, x,wvs0,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_model,rv))
    # plt.plot(x,instr_trans_model(out, x,wvs0,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_model,rv),label="model")
    # # plt.plot(x,phoenix_A0_func(wvs0)/np.max(phoenix_A0_func(wvs0))*8000)
    # # plt.plot(x,atran_2d_func(wvs0,1000)/np.max(atran_2d_func(wvs0,1000))*8000)
    # # plt.show()
...
if __name__ == "__main__":
    ...
    target_rv = 3
    phoenix_model_host_filename = glob(os.path.join(phoenix_folder, "ups_Her_lte11000-3.50-0.0.PHOENIX-ACES-AGSS-COND-2011-HiRes.fits"))[0]
