...
if __name__ == "__main__":
    ...
    if 1:
        ...
        science_lw = hdulist[0].data[:, :, 1]
        print(science_lw.shape)
        wmp_dwvs = wvs_per_orders[:, 1:wvs_per_orders.shape[-1]] - wvs_per_orders[:, 0:wvs_per_orders.shape[-1]-1]
        wmp_dwvs = np.concatenate([wmp_dwvs, wmp_dwvs[:, -1][:, None]], axis=1)
        science_lw_wvunit = science_lw * wmp_dwvs
        line_width_func = interp1d(np.ravel(wvs_per_orders), np.ravel(science_lw_wvunit), bounds_error=False,
                                   fill_value=np.nan)
        ...
        # atran_filename = os.path.join("/scr3/jruffio/data/kpic/","models","atran","atran_13599_30_5000_2_25.13_1.9_2.6.dat")
        #
        ...
        #     atran_pixel_widths = np.array(pd.DataFrame(pixel_width_func(atran_wvs_cropped)).interpolate(method="linear").fillna(method="bfill").fillna(method="ffill"))[:, 0]
        #     atran_spec_conv = convolve_spectrum_line_width(atran_wvs_cropped,atran_spec_cropped,atran_line_widths,mypool=mypool)
        #     atran_spec_conv = convolve_spectrum_pixel_width(atran_wvs_cropped,atran_spec_conv,atran_pixel_widths,mypool=mypool)
        #
        ...
        #         plt.plot(wvs_per_orders[order_id,:],spectra[fileid,order_id,:],alpha=0.5,linewidth=1,label=os.path.basename(filename))
        ...
        #     tmp = np.array(
        #         pd.DataFrame(np.concatenate([myvec_cp, myvec_cp[::-1]], axis=0)).interpolate(method="linear").fillna(
        #             method="bfill").fillna(method="ffill"))
        #     myvec_cp_lpf = np.array(
        #         pd.DataFrame(tmp).rolling(window=window, center=True).median().interpolate(method="linear").fillna(
        #             method="bfill").fillna(method="ffill"))[0:np.size(myvec), 0]
        ...
        #     LPF_fftmyvec[370:420] = fftmyvec[370:420]
        #     LPF_fftmyvec[2 * np.size(myvec_cp)-420:2 * np.size(myvec_cp)-370] = fftmyvec[2 * np.size(myvec_cp)-420:2 * np.size(myvec_cp)-370]
        ...
        #
        #     plt.figure(1)
        #     plt.plot(np.abs(fftmyvec))
        #
        #     plt.figure(2)
        #     plt.plot( myvec, label="original")
        #     plt.plot(LPF_myvec, label="lpf")
        #     plt.plot(HPF_myvec, label="hpf")
        #     plt.legend()
        #     plt.show()
        #
        ...
        #
        #
        # lpf,hpf = LPFvsHPF(med_spec[3,:], 20)
