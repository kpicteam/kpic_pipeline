...
if __name__ == "__main__":
    ...
    if 1:
        ...
        if datatype == "science":
            background_filelist = glob(os.path.join(kap_And_dir,"background_science","*.fits"))
        elif datatype == "star":
            background_filelist = glob(os.path.join(kap_And_dir,"background_star","*.fits"))

        background600_list = []
        for filename in background_filelist:
        ...
        out = os.path.join(kap_And_dir,"calib","background_{0}_cube.fits".format(datatype))
        try:
        ...
        out = os.path.join(kap_And_dir,"calib","background_{0}_med.fits".format(datatype))
        try:
            hdulist.writeto(out, overwrite=True)
        except TypeError:
            hdulist.writeto(out, clobber=True)
        ...
        out = os.path.join(kap_And_dir,"calib","background_{0}_badpixmap.fits".format(datatype))
        try:
            hdulist.writeto(out, overwrite=True)
        except TypeError:
            hdulist.writeto(out, clobber=True)
