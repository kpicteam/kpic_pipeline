...
if __name__ == "__main__":
    ...
    for fiber_num in np.arange(1,4):
        star_filelist = glob(os.path.join(kap_And_dir, "calib", "*_backgroundflux_extract_f{0}.fits".format(fiber_num)))
        # print(star_filelist)
        # exit()
        ...
        for fileid,filename in enumerate(star_filelist):
            ...
            # exit()
            out = os.path.join(kap_And_dir, "calib", os.path.basename(filename).replace("_backgroundflux_extract_f","_backgroundflux_extract_res_f"))
            out = glob(out)[0]
            hdulist = pyfits.open(out)
        ...
        star_spectra[:,:,0:170] = np.nan
        star_spectra[:,:,(2048-175)::] = np.nan
        star_background[:,:,0:170] = np.nan
        star_background[:,:,(2048-175)::] = np.nan
        star_rn[:,:,0:170] = np.nan
        star_rn[:,:,(2048-175)::] = np.nan
        ...
        #         star_wvs[fileid,order_id,:] = wvs
        #         plt.plot(wvs,star_spectra[fileid,order_id,:],alpha=0.5,linewidth=1)
        for order_id in range(9):
            plt.subplot(9, 1, 9-order_id)
            wvs = np.polyval(polycoefs_wavsol_jason[8-order_id,:],np.arange(0,2048))
            plt.plot(wvs,star_med_spec[order_id,:],linestyle="--",linewidth=2)
            plt.ylim([0,2*np.nanmedian(star_med_spec[order_id,:])])
        # plt.tight_layout()
        ...
        #     for order_id in range(9):
        #         plt.subplot(9, 1, 9-order_id)
        #         plt.plot(star_rn_ratio[fileid,order_id,:],alpha=0.5,linewidth=1)
        # plt.figure(3)
        # for fileid, filename in enumerate(star_filelist):
        #     for order_id in range(9):
        #         plt.subplot(9, 1, 9-order_id)
        ...
        hdulist.append(pyfits.PrimaryHDU(data=star_spectra))
        out = os.path.join(kap_And_dir, "calib", os.path.basename(star_filelist[0]).replace( "_backgroundflux_extract","_background_spectra"))
        try:
        ...
        out = os.path.join(kap_And_dir, "calib", os.path.basename(star_filelist[0]).replace( "_backgroundflux_extract","_background_wvs"))
        try:
            hdulist.writeto(out, overwrite=True)
        except TypeError:
            hdulist.writeto(out, clobber=True)
