...
def _extract_flux(paras):
    ...
    for k in range(nxstamp):
        ...
        if not skip:
            tmp_datacol = datacol[np.max([int(np.round(y0))-5,0]):np.min([int(np.round(y0))+5,np.size(datacol)])]
...
if __name__ == "__main__":
    ...
    star_filelist = glob(os.path.join(kap_And_dir, "background_science", "*.fits"))
    ...
    for filename in star_filelist:
        ...
        # plt.clim([0,20])
        flattened = np.nanmean(star_im_skysub*background600_badpixmap,axis=1)
    ...
    for star_im_id in range(star_cube.shape[0]):
        for fiber_num in np.arange(1,4):
            ...
            background600_badpixmap[np.where(np.abs(old_residuals)>3)]=np.nan
            ...
            for order_id,(y1,y2) in enumerate(fibers[fiber_num]):
                ...
                if 0:
                    ...
                    for k in range(N_chunks-1):
                        ...
                        badpix_chunks.append(background600_badpixmap[y1:y2,k*chunk_size:(k+1)*chunk_size])
                    ...
                    badpix_chunks.append(background600_badpixmap[y1:y2,(N_chunks-1)*chunk_size:nx])
            ...
            out = os.path.join(kap_And_dir, "calib", os.path.basename(star_filelist[star_im_id]).replace(".fits","_backgroundflux_extract_f{0}.fits".format(fiber_num)))
            try:
            ...
            out = os.path.join(kap_And_dir, "calib", os.path.basename(star_filelist[star_im_id]).replace(".fits","_backgroundflux_extract_res_f{0}.fits".format(fiber_num)))
            try:
                hdulist.writeto(out, overwrite=True)
            except TypeError:
                hdulist.writeto(out, clobber=True)
