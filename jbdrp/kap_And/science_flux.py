...
if __name__ == "__main__":
    ...
    star_filelist = glob(os.path.join(kap_And_dir, "science", "*.fits"))
    ...
    for filename in star_filelist:
        ...
        star_im_skysub = star_im-background600_med
    ...
    for star_im_id in range(star_cube.shape[0]):
        ...
        #     fiber_num = 2
            ...
            out = os.path.join(kap_And_dir, "calib", os.path.basename(star_filelist[star_im_id]).replace(".fits","_scienceflux_extract_f{0}.fits".format(fiber_num)))
            try:
            ...
            out = os.path.join(kap_And_dir, "calib", os.path.basename(star_filelist[star_im_id]).replace(".fits","_scienceflux_extract_res_f{0}.fits".format(fiber_num)))
            try:
                hdulist.writeto(out, overwrite=True)
            except TypeError:
                hdulist.writeto(out, clobber=True)
