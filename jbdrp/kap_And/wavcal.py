...
def wavcal_model_poly(paras, x,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont):
    c_kms = 299792.458
    if 0:
        wvs_coefs = paras[0:deg_wvs+1]
        cont_coefs = paras[deg_wvs+1:deg_wvs+1+deg_cont+1]
        star_rv = paras[deg_wvs+1+deg_cont+1]
        water = paras[deg_wvs+1+deg_cont+1+1]
    else:
        wvs_coefs = paras[0:deg_wvs+1]
...
def wavcal_nloglike_poly(paras, x,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont):
    # A, w, y0, B, rn, g = paras
    ...
    m = wavcal_model_poly(paras, x,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont)
    if m is not None:
...
def _fit_wavcal_poly(paras):
    x,wvs0,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont = paras
    dwv = 0.3/2048
    if 0:
        if 0:
            ...
            for k, offset in enumerate(offsets):
                ...
                m = wavcal_model_poly(paras0, x, spectrum, spec_err, phoenix_HR8799_func, atran_2d_func, deg_wvs, 1)
            ...
            for k,wv_min in enumerate(wvs_min):
                ...
                for l, wv_mid in enumerate(wvs_mid):
                    for m, wv_max in enumerate(wvs_max):
                        nloglike_arr[k,l,m] = wavcal_nloglike_poly([wv_min,wv_mid,wv_max,5000], x, spectrum, spec_err, phoenix_HR8799_func, atran_2d_func, 2, deg_cont)
    ...
    # exit()
    res = minimize(lambda paras: wavcal_nloglike_poly(paras, x,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont), paras0, method="nelder-mead",
    ...
    # plt.plot(x,wavcal_model_poly(out, x,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont),label="model")
    # plt.plot(x,spectrum,label="data")
    # plt.fill_between(x,spectrum-spec_err,spectrum+spec_err,label="data err",alpha=0.5)
    # plt.legend()
    # plt.figure(2)
    # out[-1] = 3000
    # plt.plot(x, wavcal_model_poly(out, x,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont), label="res {0}".format(out[-1]))
    # out[-1] = 4000
    # plt.plot(x, wavcal_model_poly(out, x,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont), label="res {0}".format(out[-1]))
    # out[-1] = 5000
    # plt.plot(x, wavcal_model_poly(out, x,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont), label="res {0}".format(out[-1]))
    ...
    return np.dot(M2,out[0:deg_wvs+1]),out[0:deg_wvs+1],M2,out[deg_wvs+1]
...
if __name__ == "__main__":
    ...
    if 1:
        ...
        if 0:
            ...
            phoenix_HR8799_conv = convolve_spectrum_pixel_width(phoenix_wvs, phoenix_HR8799, phoenix_pixel_widths,mypool=mypool)
        ...
        if 1:
            for k in range(len(star_filelist)):
                ...
                new_wvs,coefs,M,water = _fit_wavcal_poly((x,wvs0,spectrum,spec_err, phoenix_HR8799_func,atran_2d_func,deg_wvs,deg_cont))
                # np.dot(M2,out[0:deg_wvs+1]),out[0:deg_wvs+1],M2,out[deg_wvs+1]
                print(out)
                plt.subplot(1, 2, 1)
                plt.plot(spectrum/np.nanmax(spectrum),label="spec")
                plt.plot(atran_2d_func(wvs0,water),label="Jason's wavcal")
                plt.plot(atran_2d_func(new_wvs,water),label="new wavcal")
                plt.legend()
                plt.show()
                # print(out,residuals)
                exit()
            ...
            for kl, out in zip(kl_list, outputs_list):
                new_wvs,coefs,M,water = out
                new_wvs_arr[kl[0],kl[1],:] = new_wvs
        # print(spectra.shape)
        # exit()

        med_new_wvs = np.mean(new_wvs_arr,axis=0)
        ...
        hdulist.close()
        hdulist = pyfits.HDUList()
        hdulist.append(pyfits.PrimaryHDU(data=maxdev_new_wvs))
        out = os.path.join(kap_And_dir, "calib", "wvs_maxdev_f{0}.fits".format(fiber2extract_num))
        print(out)
        try:
            hdulist.writeto(out, overwrite=True)
        except TypeError:
