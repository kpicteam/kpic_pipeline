        ...
        if N_d != 0:
            ...
            for fib in range(Nfib):
                if np.isfinite(line_width[fib,k]) and np.isfinite(ycenter[fib,k]):
                    ...
                    if np.size(where_finite[0]) >= 7 and not np.sum(np.isnan(datacol[y0int-1:y0int+2]))>=1:
            ...
            # print(maxres,mad_std(residuals[:, k][np.where(np.isfinite(residuals[:, k]))]),np.abs((maxres) / mad_std(residuals[:, k][np.where(np.isfinite(residuals[:, k]))])))
            ...
            # plt.plot(profile_model([A[3],line_width[3,k],ycenter[3,k],0],yindices)+background,label="m4")
            # plt.legend()
            # plt.subplot(1,2,2)
