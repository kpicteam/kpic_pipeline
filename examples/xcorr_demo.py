...
target_dir = os.path.join(main_dir,"20200702_ROXs_42Bb") # the star of interest
calib_dir = os.path.join("../../public_kpic_data/", "20200702_HIP_81497/calib") # calib subdir
raw_data_dir = os.path.join(target_dir, "raw") # raw 2D images

# select science files
filestr = "nspec200702_0{0:03d}.fits"
filenums =  [65, 66, 67, 68]
filelist = [os.path.join(raw_data_dir, filestr.format(i)) for i in filenums]

# load wavelength solution
with fits.open(os.path.join("../../public_kpic_data/utils/", "first_guess_wvs_20200607_HIP_81497.fits")) as hdulist:
...
all_errors = []
for num in filenums:
    filepath = os.path.join(target_dir + "nspec200702_0{0:03d}_fluxes.fits".format(num))
...
template_filename = os.path.join(target_dir, "lte015.5-4.5-0.0a+0.0.BT-Settl.spec.7")
